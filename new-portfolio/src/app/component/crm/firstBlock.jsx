  ...
  return (
    <section className={css.firtBlockAbout}>
      <div className={css.listLandignd}>
        ...
        <Link href={`/${lng}/crm`} className={css.activLink}>
          {t("fiveOfferName")}
        </Link>
      </div>
      <div className={css.blockInFirst}>
        <div className={css.advantageWr}>
          <h1 className={css.advantTitleM}>{t("titleCrm")}</h1>
          <p className={css.advantDescriptM}>{t("descCrm")}</p>
        </div>
        <Image src={firstImage} className={css.firstImageSt} alt="Photo" />
      </div>
      <p className={css.appMainPT}>{t("firstTitleCrm")}</p>
      <p className={css.appMaitPdescription}>{t("firstDescCrm")}</p>
    </section>
  );
};
